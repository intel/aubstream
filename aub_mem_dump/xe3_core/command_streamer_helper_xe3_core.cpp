...
void GpuXe3Core::initializeDefaultMemoryPools(AubStream &stream, uint32_t devicesCount, uint64_t memoryBankSize) const {
...
CommandStreamerHelper &GpuXe3Core::getCommandStreamerHelper(uint32_t device, EngineType engineType) const {
...
const MMIOList GpuXe3Core::getGlobalMMIO() const {
    const MMIOList globalMMIO = {
        ...
        MMIOPair(0x0000e7c8, 0x00000000),     // LSC_CHICKEN_BIT_0 bits 0:31
