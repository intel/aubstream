...
TEST_F(CommandStreamerHelperTest, givenNvlpAndMemoryAlignment4KbWhenCheckIsMemorySupportedThenOnlyForMemoryBankSystemIsReturnedTrue) {
...
TEST_F(CommandStreamerHelperTest, givenNvlpAndMemoryAlignment64KbWhenCheckIsMemorySupportedThenOnlyForMemoryBankSystemIsReturnedTrue) {
    TEST_REQUIRES(gpu->productFamily == ProductFamily::Nvlp);
    auto &rcs = gpu->getCommandStreamerHelper(defaultDevice, ENGINE_RCS);
...
TEST_F(CommandStreamerHelperTest, givenNvlpAndMemoryAlignment1bWhenCheckIsMemorySupportedThenReturnFalse) {
    TEST_REQUIRES(gpu->productFamily == ProductFamily::Nvlp);
    auto &rcs = gpu->getCommandStreamerHelper(defaultDevice, ENGINE_RCS);
    auto pageSize = 1u;
