...
TEST(Gpu, givenXe3WhenInitializingGlobalMmiosThenProgramMocsAndPatIndex) {
    ...
    EXPECT_CALL(stream, writeMMIO(0x4820, 0b0000'0000'1100, 0xffffffff)).Times(1);
    ...
    EXPECT_CALL(stream, writeMMIO(0x00007004, 0xffff0000, 0xffffffff)).Times(1);
    EXPECT_CALL(stream, writeMMIO(0x00009008, 0x00000200, 0xffffffff)).Times(1);
    EXPECT_CALL(stream, writeMMIO(0x0000900c, 0x00001b40, 0xffffffff)).Times(1);
    EXPECT_CALL(stream, writeMMIO(0x0000b120, 0x14000002, 0xffffffff)).Times(1);
    EXPECT_CALL(stream, writeMMIO(0x0000b134, 0xa0000000, 0xffffffff)).Times(1);
    EXPECT_CALL(stream, writeMMIO(0x0000b234, 0xa0000000, 0xffffffff)).Times(1);
    EXPECT_CALL(stream, writeMMIO(0x0000ce90, 0x00030003, 0xffffffff)).Times(1);
    EXPECT_CALL(stream, writeMMIO(0x0000cf58, 0x80000000, 0xffffffff)).Times(1);
    EXPECT_CALL(stream, writeMMIO(0x0000e194, 0xffff0002, 0xffffffff)).Times(1);
    EXPECT_CALL(stream, writeMMIO(0x00014800, 0x00030003, 0xffffffff)).Times(1);
    ...
    EXPECT_CALL(stream, writeMMIO(0x0000e7c8, 0x00000000, 0xffffffff)).Times(1);
