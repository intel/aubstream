...
macro(SET_FLAGS_FOR CORE_TYPE)
  foreach(SKU_NAME ${ARGN})
    if(SUPPORT_${SKU_NAME})
      set(AUBSTREAM_SUPPORT_${SKU_NAME} TRUE CACHE BOOL "Support ${SKU_NAME} in aubstream")
      if(NOT SUPPORT_${CORE_TYPE})
        message(STATUS "Auto-Enabling aubstream ${CORE_TYPE} support for ${SKU_NAME}")
        set(AUBSTREAM_SUPPORT_${CORE_TYPE} TRUE CACHE BOOL "Support ${CORE_TYPE} devices" FORCE)
      endif()
    endif()
    if(AUBSTREAM_TESTS_${SKU_NAME})
      set(AUBSTREAM_TESTS_${SKU_NAME} TRUE CACHE BOOL "Build ULTs for ${SKU_NAME} in aubstream")
      if(NOT TESTS_${CORE_TYPE})
        message(STATUS "Auto-Enabling aubstream ${CORE_TYPE} tests for ${SKU_NAME}")
        set(AUBSTREAM_TESTS_${CORE_TYPE} TRUE CACHE BOOL "Build ULTs for ${CORE_TYPE} in aubstream" FORCE)
      endif()
    endif()
  ...
  set(AUBSTREAM_SUPPORT_${CORE_TYPE} ${SUPPORT_GEN_DEFAULT} CACHE BOOL "Support ${CORE_TYPE} devices in aubstream")
  set(AUBSTREAM_TESTS_${CORE_TYPE} ${AUBSTREAM_SUPPORT_${CORE_TYPE}} CACHE BOOL "Build ULTs for ${CORE_TYPE} devices in aubstream")

  if(NOT AUBSTREAM_SUPPORT_${CORE_TYPE} OR NOT aubstream_build_tests)
    set(AUBSTREAM_TESTS_${CORE_TYPE} FALSE)
  endif()

  if(AUBSTREAM_SUPPORT_${CORE_TYPE})
    ...
    foreach(${CORE_TYPE}_PLATFORM ${ARGN})
      set(AUBSTREAM_SUPPORT_${${CORE_TYPE}_PLATFORM} ${SUPPORT_PLATFORM_DEFAULT} CACHE BOOL "Support ${${CORE_TYPE}_PLATFORM} in aubstream")
      if(AUBSTREAM_TESTS_${CORE_TYPE})
        set(AUBSTREAM_TESTS_${${CORE_TYPE}_PLATFORM} ${AUBSTREAM_SUPPORT_${${CORE_TYPE}_PLATFORM}} CACHE BOOL "Build ULTs for ${${CORE_TYPE}_PLATFORM} in aubstream")
      endif()
      if(NOT AUBSTREAM_SUPPORT_${${CORE_TYPE}_PLATFORM} OR NOT AUBSTREAM_TESTS_${CORE_TYPE} OR NOT aubstream_build_tests)
        set(AUBSTREAM_TESTS_${${CORE_TYPE}_PLATFORM} FALSE)
      endif()
    endforeach()
  endif()
...
macro(DISABLE_FLAGS_FOR CORE_TYPE)
  set(AUBSTREAM_SUPPORT_${CORE_TYPE} FALSE CACHE BOOL "Support ${CORE_TYPE} devices in aubstream" FORCE)
  set(AUBSTREAM_TESTS_${CORE_TYPE} FALSE CACHE BOOL "Build ULTs for ${CORE_TYPE} devices in aubstream" FORCE)
  foreach(SKU_NAME ${ARGN})
    set(AUBSTREAM_SUPPORT_${SKU_NAME} FALSE CACHE BOOL "Support ${SKU_NAME} in aubstream" FORCE)
    set(AUBSTREAM_TESTS_${SKU_NAME} FALSE CACHE BOOL "Build ULTs for ${SKU_NAME} in aubstream" FORCE)
  endforeach()
endmacro()
...
macro(ENABLE_ADDITIONAL_SKU)
  foreach(SKU_NAME ${ARGN})
    set(AUBSTREAM_SUPPORT_${SKU_NAME} TRUE CACHE BOOL "Support ${SKU_NAME}" FORCE)
  endforeach()
endmacro()

macro(TEST_ADDITIONAL_SKU)
  foreach(SKU_NAME ${ARGN})
    set(AUBSTREAM_TESTS_${SKU_NAME} TRUE CACHE BOOL "Build ULTs for ${SKU_NAME}" FORCE)
  endforeach()
endmacro()
...
GET_AVAILABLE_PLATFORMS("SUPPORTED" "SUPPORT" ALL_AUBSTREAM_AVAILABLE_SUPPORTED_PLATFORMS)
GET_AVAILABLE_PLATFORMS("TESTED" "TESTS" ALL_AUBSTREAM_AVAILABLE_TESTED_PLATFORMS)

# Output platforms
message(STATUS "[Aubstream] All supported platforms: ${ALL_AUBSTREAM_AVAILABLE_SUPPORTED_PLATFORMS}")
message(STATUS "[Aubstream] All tested platforms: ${ALL_AUBSTREAM_AVAILABLE_TESTED_PLATFORMS}")
